...
for (k in 1:nn){
  ...
  results[[k]] <- foreach(i = iter(seq_along(Ysim)),
...
#---- Save Results
save(results, file = "Results/1.1_SampleSize.RData")

#-------------------------------------------
#    Result summary
#-------------------------------------------

results <- lapply(results, "[", , c(2:1, 3:6), )
...
rmse_df <- data.frame(
  method = factor(rep(method_list, nn), levels = method_list),
  n = rep(nvec, each = nm),
  rmse = c(rmse_all), se = c(rmse_se))
rmse_df <- subset(rmse_df, n > 50)
...
plotlay <- ggplot(rmse_df, aes(x = n, group = method, 
  ...
  scale_y_continuous(name = "RMSE", #trans = "log10", 
    limits = c(0, .3)) + 
  scale_x_continuous(name = "Sample size") + #, trans = "log10")
